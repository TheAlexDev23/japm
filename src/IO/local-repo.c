...
void add_package_to_local_repo(package pkg, const char *package_name)
{
...
start_again:;
    ...
    char *sql = malloc(sizeof(char) *
                       (strlen("SELECT * FROM packages WHERE name = ''") + strlen(package_name) + strlen(";") + 1));
...
void update_package_in_local_repository(package pkg)
{
    ...
    char *sql = malloc(sizeof(char) *
                       (strlen("UPDATE packages SET version = ") + strlen(json_object_get_string(pkg.version)) + strlen(", description = ") + strlen(json_object_get_string(pkg.description)) + strlen(", dependencies = ") + strlen(json_object_get_string(pkg.dependencies)) + strlen(", remove_instructions = ") + strlen(json_object_get_string(pkg.remove)) + strlen(", update_instructions = ") + strlen(json_object_get_string(pkg.update)) + strlen(" WHERE name = ") + strlen(json_object_get_string(pkg.name)) + strlen(";") + 1));
